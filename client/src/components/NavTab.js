...
export function NavTab({ currentUser }) {
  ...
  function NavLinkBtn({ path, children }) {
    return (
      <NavLink
        ...
        className={({ isActive }) =>
          isActive ? 'bg-black text-white rounded shadow-xl' : ''
        }
      >
  ...
  useEffect(() => {
    window.onscroll = () =>
      window.pageYOffset < window.screen.height
        ? setTypeDark('text-black')
        : setTypeDark('text-white');
  ...
  return (
    <div
      className="sticky top-0 md:flex flex-col hidden bg-white"
      style={{ height: '94vh' }}
    >
      ...
      <div
        className={`absolute -bottom-10 right-5 font-semibold text-3xl ${typeDark}`}
      >
        Murdoc.
      </div>
    </div>
